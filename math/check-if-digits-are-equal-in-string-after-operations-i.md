...
firstDigit = $$\sum_{i = 0, r = 0}^{n - 2, n - 2} \binom{n-2}{r} * s[i]\ \%\ 10$$
...
To find large $$ \binom{n}{r} \ \% \ p $$, if $$ p $$ is a prime number, then we can break the large combination into parts and calculate the final result.
...
$$
Where $$
N=  n_0 . n_1 ... n_k , \ N_i = \frac{N}{n_0 . n_1 ... n_{i - 1} . n_{i + 1} ... n_k}
$$ and $$ z_i = N^{-1}_i $$
